...
const state = {
  ...
  ufoBullets: [],
  fx: [], // vector debris lines for explosions
...
function makeBullet(x,y,vx,vy){
  return { x,y,vx,vy, life: state.mods.bulletLife, r: 2.4 };
}

function makeUfoBullet(x,y,vx,vy){
  return { x,y,vx,vy, life: 2.2, r: 3.0 };
}
...
function spawnExplosion(x,y,power=1,col=COL.green){
  ...
  for (let i=0;i<count;i++){
    ...
    state.fx.push({
      ...
      vy: Math.sin(a)*sp,
      len,
      life: rand(0.25, 0.85) * (0.9 + power*0.15),
      col
    });
  }
}
