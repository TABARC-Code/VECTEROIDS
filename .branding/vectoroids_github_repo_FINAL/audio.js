...
// window.VO_AUDIO_DEBUG.lastError will tell you what broke.
...
(function(){
  ...
  async function arm(){
    try{
      ...
      // Preload a few so first shot/explosion isn't silent.
      await Promise.allSettled([ load('fire'), load('expSmall'), load('hiss') ]);
      startHiss();

      // If debug overlay exists, hide it on successful arm.
      const el = dbgEl();
